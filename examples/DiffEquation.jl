...
diff = IdentityOperator(span(B))
BC = DirichletBC(x->0);

fD = x->x;
Diff = differentiation_operator(span(B))^2
DE = DiffEquation(span(B),Dom,Diff,fD, (BC,BC));
...
diff = differentiation_operator(span(B))
NeumannBC(x->0.)

fN = x->x;
Diff = differentiation_operator(span(B))^2
DE = DiffEquation(span(B),Dom,Diff,fN, (BC,));
...
B2 = span(FourierBasis(31,-1.0,1.0)⊗FourierBasis(31,-1.0,1.0))
...
B2 = span(FourierBasis(21,-1.0,1.0)⊗FourierBasis(21,-1.0,1.0))
...
using StaticArrays
B2 = span(FourierBasis(41,-1.0,1.0)⊗FourierBasis(41,-1.0,1.0))
