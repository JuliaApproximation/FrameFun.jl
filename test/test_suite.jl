...
Test.with_handler(custom_handler) do
    ...
    for funtype in (ExpFun, ChebyFun)
        ...
        for D in [Interval(-1.5,0.7) Interval(-1.5,-0.5)+Interval(0.5,1.5)]
            ...
            for solver_type in (FE.FE_ProjectionSolver, FE.FE_DirectSolver)
                ...
                for n in [FE.default_fourier_n(D) 100]
                    println("\tN = $n")
    ...
    f(x)=cos(x.^2)-BigFloat(1.0)
    g(x)=Complex{BigFloat}(1.0im)*cos(x.^2)-BigFloat(1.0)
    # Some BigFloat tests (ExpFun/DirectSolver only)
    for D in [Interval(BigFloat(-1.5),BigFloat(0.7)) Interval(BigFloat(-1.5),BigFloat(-0.5))+Interval(BigFloat(0.5),BigFloat(1.5))]
        show(D); print("\n")
        for T in [BigFloat(1.7) FE.default_fourier_T(D) BigFloat(2.3)]
            print("T = $T \t")
            for func in (f,g)
                F=@timed(ExpFun(func,D,FE.FE_DirectSolver,n=35,T=T))
                @printf("%3.2e s\t %3.2e bytes",F[2],F[3])
