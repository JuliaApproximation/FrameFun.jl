...
immutable FE_ProjectionSolver{ELT} <: FE_Solver{ELT}
    ...
    function FE_ProjectionSolver(op::AbstractOperator; cutoff = default_cutoff(op), trunc = TruncatedSvdSolver, R = estimate_plunge_rank(op), options...)
        plunge_op = plunge_operator(op)
        ...
        x1 = zeros(ELT, src(op))
        x2 = zeros(ELT, src(op))
        new(TS, op, plunge_op, b,blinear,x1,x2)
    end
end

FE_ProjectionSolver(op::AbstractOperator; options...) =
    FE_ProjectionSolver{eltype(op)}(op; options...)


function plunge_operator(op)
    A = op
    Ap = op'
